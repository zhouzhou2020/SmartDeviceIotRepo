package org.debug.smartdeviceiot.server.signalprocess.filtering;
...
import org.debug.smartdeviceiot.server.signalprocess.filtering.type.FilterFunction;
import org.debug.smartdeviceiot.server.signalprocess.filtering.window.WindowFunction;
...
public class Filter {
    ...
    public List<Signal> apply(Signal signal) {
        ...
        for (int i = 0; i < 5; i++) {
            System.out.println(i+": "+signal.getValues()[i]);
        }
        ...
        for (int i = 0; i < M; i++) {
            if (i == (M - 1) / 2) {
                ...
                newValues[i] = Math.sin((2.0 * Math.PI * (i - (M - 1)*1.0 / 2)) / K) / (Math.PI * (i - 1.0*(M - 1) / 2));
            }
